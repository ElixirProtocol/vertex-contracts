...
interface IProductEngine is IProductEngineState {
    ...
    function initialize(address _clearinghouse, address _quote, address _endpoint, address _admin, address _fees)
        external;
    ...
    function swapLp(uint32 productId, int128 amount, int128 priceX18, int128 sizeIncrement, int128 lpSpreadX18)
        external
        returns (int128, int128);

    function swapLp(uint32 productId, int128 baseDelta, int128 quoteDelta) external returns (int128, int128);
    ...
    function decomposeLps(bytes32 liquidatee, bytes32 liquidator, address feeCalculator) external returns (int128);
}
